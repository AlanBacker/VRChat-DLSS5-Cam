...
| DLSS 5 | 강도 | 뉴럴 패스의 전체 강도(0–2). 1까지는 런타임 자체의 강도이며, 1을 넘으면 앱이 뉴럴 결과와 원본의 차이를 증폭합니다(아티팩트가 커질 수 있음). |
| DLSS 5 | 전역 톤 / 지역 톤 | 전역 및 지역 톤 매핑 강도(0–1, 런타임은 강도를 이 범위로 제한합니다). |
| DLSS 5 | 지역 구조 / 피부 구조 | 디테일 강화(0–1). 피부 구조는 런타임 기본값으로 둘 수 있습니다. |
...
완성된 화면은 세 개의 디스플레이 버퍼와 큐 간 펜스 대기를 통해 전달되므로 미리보기는 항상 가장 최근에 완성된 프레임을 보여 주고 창은 뉴럴 패스를 기다리지 않습니다(`src/core/App.cpp`, `src/gfx/Device.cpp`).
