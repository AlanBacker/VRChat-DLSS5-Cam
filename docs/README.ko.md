...
- **미디어 라이브러리.** 열거나 창에 끌어다 놓은 모든 이미지와 동영상이 미리보기 아래의 썸네일 줄에 들어갑니다. 하나를 클릭하면 모든 슬라이더가 적용된 상태로 미리 볼 수 있고,
  처리할 항목에 체크한 뒤 *모두 처리* 또는 *선택 항목 처리*를 누르면 현재 설정으로 차례로 처리되어 촬영 폴더에 저장됩니다.
...
- **평평하고 정돈된 인터페이스.** 세 단계(소스, DLSS 5, 저장)와 일상적인 컨트롤만 보이며, 상단 바의 *고급* 스위치로 톤·구조 슬라이더, 프레임 가이던스, DLAA, 출력 블렌드, 타이머가 나타납니다.
...
- 여러 파일을 다루려면 미리보기 아래의 **라이브러리**에 추가하세요(창에 끌어다 놓거나 *파일 추가* / *폴더 추가*). 각 파일에는 썸네일, 크기와 길이, 상태가 표시됩니다.
  ...
  상단 바의 *고급* 스위치는 자주 쓰지 않는 컨트롤을 보여 주며 일상적인 컨트롤은 항상 보입니다.
...
| 소스 | 저장 형식 | 동영상 파일 전용: *MP4(H.264)*, *MP4(HEVC)* 또는 *PNG 시퀀스*. MP4는 원본 프레임 속도를 유지하고 오디오 트랙이 있으면 유지합니다(AAC). |
| 소스 | 비트레이트 | MP4 인코더의 목표 비트레이트, 5–200 Mbit/s. |
...
| 라이브러리 | 파일 추가 / 폴더 추가 / 모두 처리 / 선택 항목 처리 / 제거 / 지우기 | 미리보기 아래의 썸네일. 클릭하면 미리보기, 체크박스로 처리 대상 선택, ×로 제거. 폴더를 추가하면 그 안의 지원 파일이 모두 추가됩니다. *표시* 섹션에서 줄을 숨길 수 있습니다. |
| 상단 바 | 고급 | 톤·구조·출력 블렌드 슬라이더, 프레임 가이던스, DLAA, 내부 타이머를 표시합니다. 기본값은 꺼짐. |
