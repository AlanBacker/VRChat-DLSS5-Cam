...
- **일괄 처리.** 이미지와 동영상을 원하는 만큼(또는 폴더째) 대기열에 넣고 *모두 처리*를 누르면 각 파일이 현재 설정으로 처리되어 촬영 폴더에 저장됩니다.
...
- 동영상을 처리하려면 *입력 소스*를 *동영상 파일*로 바꾸고 파일을 여세요(창에 끌어다 놓아도 됩니다). 미리보기에 첫 프레임이 표시되므로 먼저 슬라이더를 조정할 수 있습니다.
  *처리 후 동영상 저장*(또는 단축키)을 누르면 파일 전체가 처리되어 `<이름>_DLSS5_<너비>x<높이>.mp4`(또는 PNG 프레임이 담긴 `<이름>_DLSS5` 폴더)가 촬영 폴더에 저장됩니다.
  처리 속도는 GPU에 달려 있으며 뉴럴 패스보다 빨라지지는 않습니다. 진행 표시줄에 프레임 수와 남은 시간이 표시되고, *취소*하면 지금까지 기록된 부분은 남습니다.
- 여러 파일을 처리하려면 *일괄 처리* 섹션을 열고 파일이나 폴더를 추가한 뒤(파일 여러 개를 창에 끌어다 놓아도 됩니다) *모두 처리*를 누르세요.
  이미지와 동영상을 섞을 수 있으며 각 파일은 단독으로 처리할 때와 같은 방식으로 저장됩니다. 일괄 작업은 시작 시점의 설정으로 차례로 처리됩니다.
...
| 일괄 처리 | 파일 추가 / 폴더 추가 / 모두 처리 | 이미지와 동영상 대기열을 현재 설정으로 차례로 처리합니다. 폴더를 추가하면 그 안의 지원 파일이 모두 추가됩니다. |
