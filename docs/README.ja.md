...
| DLSS 5 | 強度 | ニューラルパス全体の強さ（0–2）。1 まではランタイム自身の強度、1 を超えるとネットワークが加えた変化全体をアプリが増幅します（アーティファクトも強調されることがあります）。 |
| DLSS 5 | グローバルトーン / ローカルトーン | グローバル・ローカルのトーン強度（0–2）。1 まではランタイムに渡され、1 を超えるとネットワークの変化のうち該当部分（グローバルトーンカーブ＝元の明るさに応じた明るさと色、またはローカルな低周波の明るさ）をアプリが増幅します。 |
| DLSS 5 | ローカル構造 / 肌の構造 | ディテール強調（0–2）。1 を超えると変化のうち細部の成分を増幅します（肌の構造は肌色の領域のみ）。肌の構造はランタイム既定のままにできます。 |
