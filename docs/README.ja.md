...
- **ニューラルパスは動作中なのに画面が真っ黒または変化しない** – アプリはニューラルの各フレームを入力と比較します。ニューラルセクションに「出力の変化」が表示され、ランタイムが成功を報告しながら真っ黒または未変化の画像を返すと警告します（`log.txt` では "DLSSNR output check"）。アプリを再起動し、それでも続く場合はそのランタイムビルドはこの GPU で画像を生成できません。DLSS 5 のオン/オフで機能を再作成しなくなりました。
