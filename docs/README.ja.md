...
| DLSS 5 | 強度 | ニューラルパス全体の強さ（0–2）。1 まではランタイム自身の強度、1 を超えるとニューラル結果と元画像の差分をアプリが増幅します（アーティファクトが強調される場合があります）。 |
...
完成した画像は 3 つの表示バッファとキュー間フェンス待機で受け渡されるため、プレビューは常に最新の完成フレームを表示し、ウィンドウがニューラルパスを待つことはありません（`src/core/App.cpp`、`src/gfx/Device.cpp`）。
