...
- **卡住的网络任务。** 没有按时结束的 DLSS-NR-on-AMD 任务会让这一帧一直等到端口的等待放弃为止（`InlineWaitMs` 的 2 到 2.5 倍），这一帧显示的是上一帧的结果。
  旧版本设的 1000 ms 会让这段等待达到驱动的两秒限制，视频可能以显卡设备丢失告终；设为 500 ms 后它停在 1.3 秒左右，下一帧也会等卡住的任务结束后才开始。
