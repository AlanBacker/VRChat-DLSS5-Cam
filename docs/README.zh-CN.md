...
- **一次处理多个文件。** 打开过的文件都会进入预览下方的素材库。选中部分或全部，一次处理完，
  每个文件用共同的设置或各自的单独参数。处理前还可以旋转、翻转或裁切任意一个。
...
- **为日常使用而做。** 深色和浅色外观（默认跟随 Windows），撤销和重做并附带完整的历史记录列表，自定义命名的预设，
  可搜索所有设置的搜索栏，可调整的界面布局，自动检查新版本，四种语言（English、简体中文、日本語、한국어），界面在独立线程上运行、始终流畅，
  还有命令行用于脚本化处理。
...
**Shift+点击** 从上次点击的文件起连续选择，在空白处拖动可以框选，单击空白处则取消所有选择，缩略图上的方框也能勾选，
**Ctrl+A** 选中所有可读取的文件。鼠标位于素材库上时（或最后一次点击落在素材库里时），按 **Delete** 键可以把选中的素材移出素材库，
...
预览素材库里的文件时，画面上会出现一个小工具条，用来**旋转、翻转和裁切**：向左或向右转四分之一圈，水平或垂直翻转，
或者拉出一个裁切框——拖动它的角和边（在框内拖动可以移动它），按 **Enter** 或 *应用* 保留，按 **Esc** 取消。最后一个按钮
把文件恢复原样。每个文件各自记住方向和裁切，处理和保存的结果都用旋转裁切后的画面，每一步都会记入历史记录，可以撤销。
...
- **对比** 可用分割对比（在预览里拖动分隔线）、并排或原始画面。分隔线按界面自己的帧率跟随鼠标，即使视频播放很慢
  也不会卡。在预览上滚动鼠标滚轮以光标为中心缩放，拖动平移，双击回到适应窗口。
- **预设** 由你自己创建，位于 DLSS 5 分节：*+* 把当前参数保存为一个名字，在列表里点击即可应用，每一项都可以用当前参数
  覆盖、重命名或删除。它们保存在设置文件夹的 `presets.txt` 里。
- **搜索** 设置：用侧栏顶部的搜索栏，只有匹配的控件和它们所在的分节会留下来。**Esc** 或 ✕ 清空搜索。
...
- **全屏** 用预览角落的按钮或 **F11**：整个屏幕只显示画面。鼠标移动时显示视频控制条；**Esc** 或 **F11** 退出。
  进入和退出都有淡入淡出，在 *实时*、*图片*、*视频* 之间切换也一样。
- **深色或浅色。** *显示* 分节里的 *主题*：*跟随系统* 按 Windows 的应用颜色设置切换，也可以直接选 *深色* 或 *浅色*。
- **侧栏** 可以通过边缘的长条 bar 收起：单击隐藏或显示，拖动改变侧栏宽度。素材库上方也有同样的长条 bar：单击收起或展开，
  拖动改变高度，缩略图会随之变大或变小。鼠标放上去时，两根 bar 会移动的那条边会亮起蓝色。两个尺寸都会记住，下次启动
  继续沿用。处理文件期间侧栏会锁定，并提供 *取消*。
- **帮助** 只需一次点击：顶栏的 *?* 按钮（或 *关于* 里的 *文档*）会用界面当前的语言打开这份指南。
- **启动时** 会先显示一张小卡片，上面有图标、名称、版本（预发布版本会标注 *Pre-release*）、一行状态和一条走动的
  进度条；主窗口要等到第一帧准备好才淡入出现，因此启动时不会看到空白窗口。只有打开了 *显示* 分节里的
  *启动时重新打开上次的文件*（默认关闭），上次的文件才会再次打开。
...
| 单击素材库的 bar · 拖动它 · 单击侧栏的 bar · 拖动它 | 收起或展开素材库 · 素材库高度 · 隐藏或显示侧栏 · 侧栏宽度 |
...
| DLSS 5 | 预设 | 你自己命名的 DLSS 5 参数组合：*+* 保存当前参数，在列表里点击应用，每一项都能覆盖、重命名或删除。保存在设置文件夹的 `presets.txt` 里。 |
...
| 关于 | 打开日志文件 / 打开设置文件夹 / 文档 / 项目主页 / 第三方声明 / 重置所有设置 | 版本、GPU 和驱动信息、这份指南，以及维护按钮。 |
