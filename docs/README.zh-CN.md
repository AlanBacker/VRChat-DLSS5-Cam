...
- **对比与校验。** 提供擦除、左右分屏与原图三种模式，支持滚轮缩放、拖动平移与全屏显示，可按 1:1 比例
  核对处理前后的差异。
- **面向日常使用。** 引导完成第一步的开始页、跟随 Windows 设置的深色／浅色扁平界面、把常用项以外的控件收在一起的
  *高级* 开关、带完整历史记录的撤销与重做、可自定义命名的预设、覆盖全部设置项的搜索、在窄侧栏中依然清晰易读的可调整布局、
  更新自动检查、四种界面语言（English、简体中文、日本語、한국어）、由独立线程驱动的稳定界面响应，以及用于脚本化处理的命令行。
- **占用更少资源。** 画面没有变化时，窗口每秒只重绘几次；实时相机只在 VRChat 送来新的一帧时才读取；
  深度网络只在 DLSS 5、DLAA 或深度视图需要时才加载。
...
4. 运行 `VRChatDLSS5Cam.exe`。首次启动会打开一个简短的设置向导（语言、GitHub 访问方式、各控件的用途）。之后预览中会出现应用了 DLSS 5 的相机画面，*启用 DLSS 5* 开关旁边的标记显示为 *运行中*。
5. 在 VRChat 中取好景，按 **Ctrl+Alt+P**（或使用 *拍照* 按钮）。PNG 会保存至 `图片\VRChat DLSS5 Cam`。

未打开任何内容时，预览区显示 **开始使用** 页面：三个步骤（选择输入源、调整 DLSS 5、保存），以及每种输入方式各一张卡片。
*VRChat 实时画面* 切换到实时相机，并显示是否找到了 Spout 发送端；*打开图片* 与 *打开视频* 用于打开文件。
...
- 这两项属于进阶控件：侧栏顶部的 *高级* 开关打开时（默认即打开）才会显示。
...
Radeon 版始终运行 FSR 宿主；GeForce 版直接托管 `nvngx_dlssnr.dll`。没有需要选择的地方，两个版本在发现另一家显卡时都会指向对应的另一个版本。

Radeon 版已在一块 RX 9060 XT 上运行过：720p 与 4K 静态图片、视频文件、命令行批处理，以及实时 VRChat 相机画面（帧率低于 GeForce 显卡，
见下方列表）。DLSS-NR-on-AMD
的安装器写入的 inline 预算为 200 ms，是给游戏定的，超时后这一帧显示的是上一帧的结果；本程序会在安装完成后（若启动时文件里是别的值，
则自动重启一次）把 `dlssnr_on_amd.ini` 中的 `InlineWaitMs` 设为 500，让保存下来的图片或视频帧带着自己这一帧的结果，并把其中的 `Inline`
...
- **深度网络与端口轮流使用显卡。** 端口的网络和深度网络（DirectML）不能同时占用显卡：一次神经通道若与深度网络的预热同时运行，会从 0.17 秒被拉长到 2.7 秒，超过驱动的两秒限制，显卡设备随即丢失（驱动重启，程序需要重新打开）。
  从 v1.5.0 起，程序在每次神经通道之前都等待深度网络空闲，两者不再重叠；一张图片的前几个通道会在深度网络就绪后才开始。
- **偶尔会有一帧等上一秒左右。** 在 RX 9060 XT 上，每一百次网络任务里会有几次卡住，一直到端口的等待放弃为止（`InlineWaitMs` 的 2 到 2.5 倍），这一帧显示的是上一帧的结果。
  旧版本设的 1000 ms 会让这段等待达到驱动的两秒限制，视频可能以显卡设备丢失告终；设为 500 ms 后它停在 1.3 秒左右，下一帧也会等卡住的任务结束后才开始。无论是否使用深度网络都会出现这种停顿。
...
打开或拖入的每个文件都会进入预览下方的**素材库**，*添加文件…* 与 *添加文件夹…* 可继续添加；素材库为空时只占一行，
放入第一个文件后才展开。**双击**缩略图即可在预览中打开该文件并调整参数。单击只选中该文件，**Ctrl+点击** 加选或取消一个，
**Shift+点击** 从上次点击的文件起连续选择，在空白处拖动可框选，单击空白处取消全部选择。鼠标移到缩略图上时，
左上角会出现一个方框（有文件被选中时所有缩略图都会显示），点击即可勾选；另一角的 ✕ 把它移出素材库；
**Ctrl+A** 或 **…** 菜单中的 *全选* 选中所有可读取的文件。鼠标位于素材库上时（或最后一次点击落在素材库内时），
**Delete** 键或 *删除* 按钮会把选中的素材移出素材库，磁盘上的文件不受影响。随后 *处理选中* 或 *全部处理* 会依次处理这些文件，
每个缩略图上的进度条显示进度，处理完成后状态仍然保留。窗口较窄时，标题行的按钮会缩成图标，放不下的按钮移入 **…** 菜单。
...
- **日常操作集中在三个分节：** *视频源*（输入什么）、*DLSS 5 神经渲染*（效果如何）与 *拍照*（保存到哪里），
  其后是 *显示*、*MCP* 与 *关于*；点击分节标题可以折叠该分节。侧栏顶部搜索框旁的 **高级** 开关用于显示或隐藏常用项以外的
  全部控件：色调与结构滑块、神经渲染分辨率与输出混合、*仅拍照时运行神经渲染*、*帧引导* 与 *DLAA 预处理* 两个分节、
  *垂直同步* 与处理帧率上限、带计时器的 *内部信息*，以及状态栏中各步骤的耗时。该开关默认打开；关闭后，侧栏只保留
  拍出第一张照片所需的控件。
- **顶栏** 左侧是输入源切换（*实时*、*图片*、*视频*）、表示当前状态的标记，以及神经渲染运行时出现的 *DLSS 5* 标记；
  右侧依次是撤销、重做、历史记录、帮助、界面语言和主操作按钮：*拍照*、*处理并保存 PNG* 或 *处理并保存视频*。
  底部的 **状态栏** 显示当前打开的内容及其尺寸与帧率、正在运行的任务的进度与剩余时间，以及最近保存的文件，
  旁边的按钮可在资源管理器中显示该文件。
- **所有文字都能完整显示。** 过长的名称会换到第二行而不会被截断，放不下的数值或标记会移到下一行，并排放不下的
  一对按钮会上下排列；四种语言在最窄的侧栏宽度下都是如此。
...
- **侧栏** 在点击它与预览之间的空隙时收起（空隙中间的小箭头指示方向），素材库上方的空隙也以同样方式折叠素材库。
  **空隙边缘的那条细线** 才是拖动目标——鼠标移上去时它会变蓝，光标变为调整大小的箭头，**拖动**即可改变侧栏宽度
...
| 单击侧栏旁或素材库上方的空隙 · 拖动空隙边缘的细线 | 折叠或展开该面板 · 改变其宽度或高度 |
...
走镜像站时，程序读取的是本仓库随发布自动更新的版本清单 `updates.json`。Radeon 版查询 DLSS-NR-on-AMD 最新发布和下载其安装器时同样走所选镜像站
（发布信息来自本仓库每天刷新的 `port.json`）。
...
MCP 客户端（Claude Desktop、Claude Code、Cursor 等）可以像你一样操作程序：打开文件、修改所有设置、处理媒体库、保存图片、
查看预览。在侧栏的 **MCP** 区块打开 **运行 MCP 服务器**，或者把下面的片段写进客户端的 MCP 配置，让客户端自己启动程序
...
默认服务器只监听 `127.0.0.1`，不会连接任何地方；**只读** 让客户端只能查看。

其他电脑也能把活交给它：把 **可访问范围** 设为 **局域网**，为每个机器人或每个人添加一把 **密钥**（角色 **任务**、**管理**、**只看**），
另一台电脑上的聊天机器人或脚本就能上传图片或视频，立刻得知自己在 **队列** 中的位置和轮到的时间，之后再来下载结果。
...
| 帧引导 | 深度 | AI 估计（DirectML 上的 Depth Anything V2 Small；更新间隔与网络分辨率可调）、平面、渐变或零。深度网络只在 DLSS 5、DLAA 或深度视图需要时才加载。 |
...
| 显示 | 对比 / 适应窗口或 1:1 像素 / 缩放 / 透明区域显示棋盘格 / 显示素材库 / 显示叠加信息 / 显示日志 / 垂直同步 | 预览选项。 |
...
- **"未找到 Spout 发送端" / "等待 VRChat Spout 串流…"** – 需要在 VRChat 的 Stream 相机上启用 *Spout Stream*，且相机必须处于打开状态。前者表示没有任何 Spout 发送端在运行，后者表示有发送端在运行，但所选的发送端还没有送来画面。其他 Spout 发送端会列在 *Spout 发送端* 框中。
