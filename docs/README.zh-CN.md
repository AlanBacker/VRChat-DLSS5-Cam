...
- **为日常使用而做。** 深色和浅色外观（默认跟随 Windows），所有设置都能撤销和重做，四种语言
  （English、简体中文、日本語、한국어），界面在独立线程上运行、始终流畅，还有命令行用于脚本化处理。
...
- 如果你的显卡跑实时预览太慢，打开 DLSS 5 分节里的 *仅拍照时运行神经渲染*：预览显示原始画面，每次拍照时只为这张照片运行神经渲染。
...
单击缩略图即可预览并在它上面调节滑块。选择文件的方式：在缩略图上拖动框选、勾选每个缩略图上的方框、
Ctrl+点击、Shift+点击，或者按 **Ctrl+A** 全选。然后 *处理选中* 或 *全部处理* 会依次处理这些文件；
每个缩略图上的进度条显示进度，处理完后状态仍然可见。
...
- **三个分节完成日常工作：** *视频源*（输入什么）、*DLSS 5 神经渲染*（效果如何）和 *拍照*（保存到哪里）。
  DLSS 5 分节里的 *高级* 开关显示或隐藏色调和结构滑块、输出混合、帧引导、DLAA 和计时器。
- **对比** 可用分割对比（在预览里拖动分隔线）、并排或原始画面。在预览上滚动鼠标滚轮以光标为中心缩放，
  拖动平移，双击回到适应窗口。
- **撤销和重做** 任何设置更改：**Ctrl+Z** / **Ctrl+Y**，或顶栏里的两个箭头。
- **深色或浅色。** *显示* 分节里的 *主题*：*跟随系统* 按 Windows 的应用颜色设置切换，也可以直接选 *深色* 或 *浅色*。
- **侧栏** 可以通过边缘的把手收起。处理文件期间侧栏会锁定，并提供 *取消*。
...
| `Ctrl+Z` · `Ctrl+Y` / `Ctrl+Shift+Z` | 撤销 · 重做设置更改 |
| `Ctrl+A` | 选中素材库里的所有文件（鼠标位于素材库上） |
| `空格` | 播放 / 暂停打开的视频 |
| `←` `→`（`Shift`：10 帧）· `Home` `End` | 逐帧步进 · 跳到开头 / 结尾 |
| `I` · `O` | 设置处理范围的起点 · 终点 |
| 预览上滚动滚轮 · 拖动 · 双击 | 缩放 · 平移 · 回到适应窗口 |
| 在缩略图上拖动 · `Ctrl`+点击 · `Shift`+点击 · 右键 | 框选文件 · 加选一个 · 连续选择 · 打开菜单 |
...
| DLSS 5 | 仅拍照时运行神经渲染 | 适合实时使用太慢的显卡：预览跳过神经渲染，拍照时先对 16 帧新画面运行神经渲染再保存。静态图片不受影响。 |
