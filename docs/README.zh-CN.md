...
| DLSS 5 | 神经渲染分辨率 | 设定神经渲染的运行尺寸（不超过输入）：可按*百分比*（输入的 25–100 %），或按*最大分辨率*限制其长边——这样很大的来源（如 8K）也会以固定的较小尺寸处理。其变化会放大后叠加到全分辨率画面。数值越低 GPU 负担越小，代价是最细的细节。神经放大开启时不使用。 |
...
- **图形设备已丢失** – 显卡驱动重置了显卡。Windows 给一项 GPU 任务大约两秒，长时间满载的显卡可能超过这个限制：典型情况是实时串流时 VRChat 在同一张显卡上渲染，而神经渲染又把每一帧都放大（`log.txt`："Device removed"，`DXGI_ERROR_DEVICE_HUNG`）。那一刻显卡上的所有程序都会丢失画面。从 v1.5.1 起，程序会在驱动恢复后自动重新启动并弹出提示，丢失前那次会话的日志保留为 `log.txt` 旁边的 `log-device-loss.txt`，其最后一行 `Perf:` 记录了当时的负载和显存占用。若反复出现，请降低负载：设置处理帧率上限、调低 *神经渲染分辨率* 或限制最大分辨率、关闭神经上采样到自定义分辨率，或在 VRChat 里限制帧率；也值得试试更新显卡驱动。脚本化运行（`--process`、`--headless`、`--exit-after`）则以退出码 2 结束。
