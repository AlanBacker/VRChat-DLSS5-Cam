...
- **DLSS 5 神经渲染** 直接从 `nvngx_dlssnr.dll` 加载，提供与 RenoDX 的 DLSS 5 ReShade 插件相同的参数：
  预设、风格、强度、全局色调、局部色调、局部结构、皮肤结构、自动遮罩和 UI 修正，另有输出混合（输入曝光、色调传递强度、色彩强度）。参数即时生效，也可以在定格画面上调节。
...
| DLSS 5 | 强度 | 神经渲染的整体强度，0–2。0–1 由运行库本身控制；大于 1 时程序放大网络带来的全部变化（瑕疵也可能被放大）。 |
| DLSS 5 | 全局色调 / 局部色调 | 全局与局部色调强度，0–2。0–1 交给运行库；大于 1 时程序放大网络变化中对应的部分：全局色调曲线（随原图明度变化的亮度与色彩），或局部低频光照。 |
| DLSS 5 | 局部结构 / 皮肤结构 | 细节增强，0–2；大于 1 时放大变化中的细节部分（皮肤结构只作用于肤色区域）。皮肤结构可保持运行库默认值。 |
