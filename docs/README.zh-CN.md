...
- **侧栏** 可以通过边缘的长条 bar 收起，素材库上方也有同样的长条 bar。**单击 bar** 收起或展开面板；**bar 内侧边缘那条细线**
  才是拖动用的——鼠标放上去时它会变蓝、光标变成调整大小的箭头，**拖动它**改变侧栏宽度或素材库高度，缩略图会随之变大或
  变小。两个尺寸都会记住，下次启动继续沿用。处理文件期间侧栏会锁定，并提供 *取消*。
...
| 单击 bar · 拖动 bar 边缘的细线 | 收起或展开素材库、侧栏 · 改变其高度或宽度 |
