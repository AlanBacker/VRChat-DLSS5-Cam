...
Radeon 版始终使用 FSR 宿主方式。GeForce 版中，*高级*里的*加载方式*可手动选择方式（*自动*在 Radeon 显卡上选 FSR 宿主，在 GeForce 显卡上选直接加载）。
...
| DLSS 5 | 加载方式 | *自动*：GeForce 显卡走直接加载，Radeon 显卡走 FSR 宿主。*直接加载*：直接托管 `nvngx_dlssnr.dll`。*FSR 宿主*：以原生尺寸运行一个 FSR 3.1 上下文供 DLSS-NR-on-AMD 挂接（`amd_fidelityfx_dx12.dll` 存在时显示）。Radeon 版始终使用 FSR 宿主，不显示此控件。 |
