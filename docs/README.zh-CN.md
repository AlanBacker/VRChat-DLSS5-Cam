...
- **素材库。** 打开或拖进窗口的每张图片、每段视频都会进入预览下方的缩略图条。点击一项即可预览它，所有滑块实时生效；勾选要处理的项目，
  再点 *全部处理* 或 *处理选中*，它们就会按当前设置依次处理并保存到拍照目录。
...
- **扁平、清爽的界面。** 三步走（来源、DLSS 5、保存），常用控件一目了然；顶栏的 *高级* 开关可以展开色调和结构滑块、帧引导、DLAA、输出混合和计时信息。
...
- 想处理很多文件：把它们加入预览下方的 **素材库**（拖进窗口，或点 *添加文件* / *添加文件夹*）。每个文件都显示缩略图、尺寸和时长以及状态。
  ...
  每个文件的保存方式和单独处理时相同；缩略图上的进度条显示进度，处理后状态会保留。顶栏的 *高级* 开关展开不常用的控件，常用控件始终可见。
...
| 素材库 | 添加文件 / 添加文件夹 / 全部处理 / 处理选中 / 移除 / 清空 | 预览下方的缩略图。点击预览，复选框选择要处理的项目，× 移除；添加文件夹会加入其中所有支持的文件。可以在 *显示* 一栏隐藏这一条。 |
| 顶栏 | 高级 | 展开色调、结构和输出混合滑块、帧引导、DLAA 和内部计时；默认关闭。 |
