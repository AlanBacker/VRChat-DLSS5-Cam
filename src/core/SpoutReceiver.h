...
public:
    ...
    // Empty name = automatic (prefers the VRChat sender, otherwise the active Spout sender).
    void SetRequestedSender(const std::string& name);
    const std::string& RequestedSender() const { return m_requested; }
    ...
    bool Receive(Device& device, bool& changed, bool& fresh);
...
private:
    bool CreateTexture(Device& device, UINT w, UINT h, DXGI_FORMAT fmt);
    void ReleaseTexture(Device& device);
    void ApplySenderName(const char* name);

    spoutDX                    m_spout;
    bool                       m_open = false;
    std::string                m_requested;   // user request ("" = auto)
