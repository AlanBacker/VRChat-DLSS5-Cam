...
#define VDC_STRING_LIST(X) \
    ...
    X(TipNvof,            "NVIDIA's hardware optical flow engine (Turing or newer, nvofapi64.dll from the GeForce driver; Radeon cards have no such engine); falls back to the FSR optical flow when unavailable. Grid = spacing of the hardware vectors in source pixels: 4 px is the fastest and is upsampled to per-pixel vectors, 2 px and 1 px are finer but much slower at 4K.", "NVIDIA 的硬件光流引擎（Turing 及更新架构，使用 GeForce 驱动中的 nvofapi64.dll；Radeon 显卡没有这样的引擎），不可用时回退到 FSR 光流。网格 = 硬件矢量的间距（源像素）：4 px 最快，会上采样为逐像素矢量；2 px、1 px 更精细，但在 4K 下明显更慢。", "NVIDIA のハードウェアオプティカルフローエンジン（Turing 以降、GeForce ドライバーの nvofapi64.dll。Radeon カードにはこのエンジンはありません）。利用できない場合は FSR オプティカルフローにフォールバックします。グリッド = ハードウェアベクトルの間隔（ソースピクセル）：4 px が最速でピクセル単位に補間されます。2 px / 1 px はより精細ですが 4K では大幅に遅くなります。", "NVIDIA의 하드웨어 옵티컬 플로우 엔진(Turing 이상, GeForce 드라이버의 nvofapi64.dll. Radeon 카드에는 이런 엔진이 없습니다). 사용할 수 없으면 FSR 옵티컬 플로우로 대체합니다. 그리드 = 하드웨어 벡터 간격(소스 픽셀): 4 px가 가장 빠르고 픽셀 단위로 보간됩니다. 2 px / 1 px는 더 세밀하지만 4K에서는 훨씬 느립니다.") \
    ...
    X(TipFsrFlow,         "This program\'s own optical flow, in the shape of the optical flow of AMD\'s FidelityFX SDK (MIT): the coarsest level of a luma pyramid finds the large motion, every level below refines what it inherits, a median over whole vectors runs between the levels and the vectors are checked in both directions. It runs on any card and is what the Radeon edition uses in place of the hardware engine it does not have.", "本程序自带的光流，沿用 AMD FidelityFX SDK（MIT）中光流的结构：在亮度金字塔最粗的一层找出大范围运动，逐层向下细化，层与层之间对整条矢量取中值，并对矢量做正反两个方向的核对。任何显卡都能运行，Radeon 版以它代替缺失的硬件引擎。", "本プログラム自身のオプティカルフローで、AMD FidelityFX SDK（MIT）のオプティカルフローと同じ構造です。輝度ピラミッドの最も粗いレベルで大きな動きを捉え、下の各レベルで精密化し、レベル間でベクトル単位の中央値を取り、前後両方向で照合します。どのカードでも動作し、Radeon 版ではハードウェアエンジンの代わりに使用します。", "이 프로그램 자체의 옵티컬 플로우로, AMD FidelityFX SDK(MIT)의 옵티컬 플로우와 같은 구조입니다. 휘도 피라미드의 가장 거친 레벨에서 큰 움직임을 찾고 아래 레벨마다 정밀화하며, 레벨 사이에서 벡터 전체의 중앙값을 취하고 양방향으로 확인합니다. 어떤 카드에서도 동작하며 Radeon 에디션은 없는 하드웨어 엔진 대신 이것을 사용합니다.") \
    ...
    X(TipDlaaPreset,      "DLSS render preset, passed to the runtime as chosen. K is the transformer model and the default. With the bundled 310 runtime, E and F are the older convolutional models and J, L and M are transformer variants; A to D, G to I, N and O fall back to its default. A swapped-in runtime may honour other letters.", "DLSS 渲染预设，按所选原样传给运行库。K 为 Transformer 模型，也是默认值。随附的 310 运行库中，E、F 为较早的卷积模型，J、L、M 为 Transformer 变体；A 到 D、G 到 I、N、O 会回退到其默认预设。换用其他运行库时可用的字母可能不同。", "DLSS レンダープリセット。選んだ値をそのままランタイムに渡します。K は Transformer モデルで既定値。同梱の 310 ランタイムでは E と F が旧来の畳み込みモデル、J・L・M が Transformer の派生で、A〜D・G〜I・N・O は既定にフォールバックします。別のランタイムに差し替えると使える文字が変わることがあります。", "DLSS 렌더 프리셋. 선택한 값을 그대로 런타임에 전달합니다. K는 트랜스포머 모델이자 기본값입니다. 번들된 310 런타임에서 E와 F는 이전 컨볼루션 모델, J·L·M은 트랜스포머 변형이며 A~D, G~I, N, O는 기본값으로 대체됩니다. 다른 런타임으로 바꾸면 사용 가능한 문자가 달라질 수 있습니다.") \
    ...
    X(UpscaleStatusCapped, "DLSS and the neural pass work up to 8192 pixels a side: this output is finished at %ux%u and resampled to %ux%u.", "DLSS 与神经渲染最大支持单边 8192 像素：此输出先在 %ux%u 完成，再重采样到 %ux%u。", "DLSS とニューラルパスは一辺 8192 ピクセルまでです。この出力は %ux%u で仕上げてから %ux%u にリサンプリングします。", "DLSS와 뉴럴 패스는 한 변 8192픽셀까지입니다. 이 출력은 %ux%u에서 마친 뒤 %ux%u로 리샘플링합니다.") \
    ...
    X(TipNrCaptureOnly,   "For GPUs where the neural pass is too slow for live use: between captures the preview shows the plain picture and the neural, motion and depth passes all rest, so the GPU is nearly idle. A capture (button, hotkey or timelapse) first runs them for 16 fresh frames so the temporal history converges, then saves. Still images are not affected.", "适合神经渲染跑不动实时预览的显卡：不拍照时预览显示原始画面，神经渲染、运动和深度这几步都停下来，显卡几乎空闲。按下拍照（按钮、热键或定时拍照）后先用 16 帧新画面预热让时序历史收敛，再保存。图片模式不受影响。", "ニューラルパスがリアルタイムには重すぎる GPU 向け：撮影しない間はプレビューに元の映像を表示し、ニューラル・モーション・深度の各パスをすべて休ませるので GPU はほぼアイドルになります。撮影（ボタン、ホットキー、タイムラプス）時にまず新しい 16 フレームで時間履歴を収束させてから保存します。静止画モードには影響しません。", "뉴럴 패스가 실시간에는 너무 느린 GPU용: 촬영하지 않는 동안 미리보기에는 원본 화면을 표시하고 뉴럴·모션·깊이 패스를 모두 쉬게 하므로 GPU가 거의 유휴 상태가 됩니다. 촬영(버튼, 단축키, 타임랩스) 시 먼저 새 프레임 16장으로 시간 이력을 수렴시킨 뒤 저장합니다. 정지 이미지 모드에는 영향이 없습니다.") \
    X(NrArchHint,         "The RTX 50 build only carries code for that generation, so it cannot start on an RTX 40/30/20 card; the build for those cards is runtimes\\universal\\ and is tried next.", "RTX 50 版只包含该代显卡的代码，无法在 RTX 40/30/20 上启动；这些显卡对应的版本是 runtimes\\universal\\，接下来会尝试它。", "RTX 50 向けビルドにはその世代のコードしか含まれていないため、RTX 40/30/20 では起動できません。これらのカード向けのビルドは runtimes\\universal\\ で、次に試されます。", "RTX 50용 빌드에는 그 세대의 코드만 들어 있어 RTX 40/30/20에서는 시작할 수 없습니다. 이 카드들에 맞는 빌드는 runtimes\\universal\\이며, 다음으로 시도됩니다.") \
    X(NrAllBuildsFailed,  "None of the bundled runtime builds started on this card; log.txt holds the NGX result codes. A newer graphics driver is the first thing to try, and Reload starts the choice over.", "附带的运行库没有一份能在此显卡上启动；log.txt 中记录有 NGX 结果代码。首先可尝试更新显卡驱动，重新加载会重新开始选择。", "同梱のランタイムビルドはどれもこのカードで起動しませんでした。log.txt に NGX の結果コードが記録されています。まずはより新しいグラフィックドライバーを試す価値があり、再読み込みで選択をやり直せます。", "내장 런타임 빌드가 모두 이 카드에서 시작되지 않았습니다. log.txt에 NGX 결과 코드가 남아 있습니다. 먼저 더 새로운 그래픽 드라이버를 시도해 볼 만하며, 다시 불러오기로 선택을 처음부터 다시 할 수 있습니다.") \
    X(NrOtherVendorHint,  "The bundled runtime builds are NVIDIA's and start only on GeForce RTX cards. On a Radeon card the neural pass runs through the FSR host route with DLSS-NR-on-AMD (the Radeon edition, VRChatDLSS5Cam-win64-amd.zip); on other cards the application works as a viewer and recorder.", "附带的运行库是 NVIDIA 的，只能在 GeForce RTX 显卡上启动。Radeon 显卡上神经渲染经由 FSR 宿主方式配合 DLSS-NR-on-AMD 运行（Radeon 版，VRChatDLSS5Cam-win64-amd.zip）；其他显卡上程序可作为查看器和录制工具使用。", "同梱のランタイムビルドは NVIDIA のもので、GeForce RTX でのみ起動します。Radeon では FSR ホスト方式と DLSS-NR-on-AMD でニューラルパスが動きます（Radeon 版、VRChatDLSS5Cam-win64-amd.zip）。その他のカードではビューアーおよび録画ツールとして動作します。", "내장 런타임 빌드는 NVIDIA의 것이며 GeForce RTX 카드에서만 시작됩니다. Radeon 카드에서는 FSR 호스트 방식과 DLSS-NR-on-AMD로 뉴럴 패스가 동작합니다(Radeon 에디션, VRChatDLSS5Cam-win64-amd.zip). 다른 카드에서는 뷰어와 녹화 도구로 동작합니다.") \
    X(NrOutputCheck,      "Output change", "输出变化", "出力の変化", "출력 변화") \
    X(TipNrOutputCheck,   "Mean difference between the neural output and the picture it saw, sampled on a grid (0 = no change). The app checks every frame and warns when the runtime reports success but delivers a black or unchanged picture.", "神经渲染输出与其输入画面在采样网格上的平均差异（0 = 没有变化）。程序每帧都会检查，当运行库报告成功却给出全黑或与输入相同的画面时会提示。", "ニューラル出力と入力画像のサンプルグリッド上での平均差（0 = 変化なし）。アプリは毎フレーム確認し、ランタイムが成功を報告しながら真っ黒または未変化の画像を返すと警告します。", "뉴럴 출력과 입력 화면의 샘플 격자 평균 차이(0 = 변화 없음). 앱은 매 프레임 확인하며, 런타임이 성공을 보고하면서 검은 화면이나 입력과 같은 화면을 내놓으면 경고합니다.") \
    X(NrOutBlack,         "The neural pass reports success, but its output is black. The runtime is not producing a picture on this GPU; restart the app, and if it persists this runtime build does not work here.", "神经渲染报告成功，但输出是全黑的。运行库没有在这块显卡上生成画面；请重启程序，若仍然如此，说明这份运行库版本在这里无法工作。", "ニューラルパスは成功を報告していますが、出力は真っ黒です。ランタイムがこの GPU で画像を生成していません。アプリを再起動し、それでも続く場合はこのランタイムビルドはここでは動作しません。", "뉴럴 패스는 성공을 보고하지만 출력이 검은색입니다. 런타임이 이 GPU에서 화면을 만들지 못하고 있습니다. 앱을 다시 시작하고, 계속되면 이 런타임 빌드는 여기서 동작하지 않습니다.") \
    X(NrOutSame,          "The neural pass reports success, but its output equals the input picture. The runtime is not applying the effect on this GPU; restart the app, and if it persists this runtime build does not work here.", "神经渲染报告成功，但输出与输入画面完全相同。运行库没有在这块显卡上施加效果；请重启程序，若仍然如此，说明这份运行库版本在这里无法工作。", "ニューラルパスは成功を報告していますが、出力は入力画像と同一です。ランタイムがこの GPU で効果を適用していません。アプリを再起動し、それでも続く場合はこのランタイムビルドはここでは動作しません。", "뉴럴 패스는 성공을 보고하지만 출력이 입력 화면과 동일합니다. 런타임이 이 GPU에서 효과를 적용하지 못하고 있습니다. 앱을 다시 시작하고, 계속되면 이 런타임 빌드는 여기서 동작하지 않습니다.") \
    ...
    X(RuntimeMissing,     "The runtime files are missing. The release archive carries them under runtimes\\ next to the executable: extract it again, or set a path below.", "运行库文件缺失。发布压缩包在可执行文件旁的 runtimes\\ 下附带了它们：重新解压即可，也可以在下方指定路径。", "ランタイムのファイルがありません。リリースのアーカイブには実行ファイルの隣の runtimes\\ 以下に同梱されています。展開し直すか、下でパスを指定できます。", "런타임 파일이 없습니다. 릴리스 압축 파일에는 실행 파일 옆의 runtimes\\ 아래에 들어 있습니다. 다시 풀거나 아래에서 경로를 지정할 수 있습니다.") \
    X(RuntimeVariants,    "The archive holds one build for RTX 50 (runtimes\\blackwell\\) and one for RTX 40/30/20 (runtimes\\universal\\); the one for this card is chosen at start and the other is tried when it fails. A build for a card from another vendor goes into runtimes\\other\\.", "压缩包内含 RTX 50 版（runtimes\\blackwell\\）与 RTX 40/30/20 版（runtimes\\universal\\）；启动时选择与本显卡对应的一份，失败时改试另一份。其他厂商显卡的版本放在 runtimes\\other\\。", "アーカイブには RTX 50 向け（runtimes\\blackwell\\）と RTX 40/30/20 向け（runtimes\\universal\\）のビルドが入っています。起動時にこのカードに合うほうが選ばれ、失敗した場合はもう一方が試されます。他社製カード向けのビルドは runtimes\\other\\ に置きます。", "압축 파일에는 RTX 50용 빌드(runtimes\\blackwell\\)와 RTX 40/30/20용 빌드(runtimes\\universal\\)가 들어 있습니다. 시작할 때 이 카드에 맞는 빌드가 선택되고, 실패하면 다른 빌드를 시도합니다. 다른 제조사 카드용 빌드는 runtimes\\other\\에 둡니다.") \
    X(RuntimePathMissing, "The file set here is not there; the bundled build is used instead. Clear the path to end this note.", "这里指定的文件不存在，已改用压缩包内附带的运行库。清空路径即可不再提示。", "ここで指定したファイルがありません。同梱のビルドを代わりに使用しています。パスを空にするとこの表示は消えます。", "여기에 지정한 파일이 없어 동봉된 빌드를 대신 사용합니다. 경로를 비우면 이 안내는 사라집니다.") \
    X(RuntimeBuildBlackwell, "RTX 50 build", "RTX 50 版", "RTX 50 向けビルド", "RTX 50용 빌드") \
    X(RuntimeBuildUniversal, "RTX 40/30/20 build", "RTX 40/30/20 版", "RTX 40/30/20 向けビルド", "RTX 40/30/20용 빌드") \
    X(RuntimeBuildOther,  "build for another vendor", "其他厂商版", "他社製カード向けビルド", "다른 제조사용 빌드") \
    X(RuntimeBuildExe,    "file next to the executable", "程序目录中的文件", "実行ファイルの隣のファイル", "실행 파일 옆의 파일") \
    X(RuntimeFallbackToast, "The %s failed on this GPU; switching to the %s", "%s在此显卡上失败，改用另一份：%s", "%s: この GPU では動作しませんでした。%sに切り替えます", "%s: 이 GPU에서 실패했습니다. %s로 전환합니다") \
    X(RuntimePath,        "Runtime path", "运行库路径", "ランタイムのパス", "런타임 경로") \
    X(Browse,             "Browse…", "浏览…", "参照…", "찾아보기…") \
    X(FsrHostName,        "FSR host", "FSR 宿主", "FSR ホスト", "FSR 호스트") \
    X(FsrDllMissing,      "amd_fidelityfx_dx12.dll is missing next to the executable. The Radeon edition (VRChatDLSS5Cam-win64-amd.zip) carries it.", "可执行文件旁缺少 amd_fidelityfx_dx12.dll。Radeon 版（VRChatDLSS5Cam-win64-amd.zip）自带该文件。", "実行ファイルの隣に amd_fidelityfx_dx12.dll がありません。Radeon 版（VRChatDLSS5Cam-win64-amd.zip）に同梱されています。", "실행 파일 옆에 amd_fidelityfx_dx12.dll이 없습니다. Radeon 에디션(VRChatDLSS5Cam-win64-amd.zip)에 들어 있습니다.") \
    X(AmdPortDetected,    "DLSS-NR-on-AMD: loaded (%s)", "DLSS-NR-on-AMD：已加载（%s）", "DLSS-NR-on-AMD：読み込み済み（%s）", "DLSS-NR-on-AMD: 로드됨(%s)") \
    X(AmdPortNotFound,    "DLSS-NR-on-AMD: not loaded", "DLSS-NR-on-AMD：未加载", "DLSS-NR-on-AMD：未読み込み", "DLSS-NR-on-AMD: 로드되지 않음") \
    X(AmdPortInstall,     "Install DLSS-NR-on-AMD…", "安装 DLSS-NR-on-AMD…", "DLSS-NR-on-AMD をインストール…", "DLSS-NR-on-AMD 설치…") \
    X(AmdPortUpdateTo,    "Update to %s…", "更新到 %s…", "%s に更新…", "%s(으)로 업데이트…") \
    X(AmdPortRerun,       "Run the installer again…", "再次运行安装器…", "インストーラーを再実行…", "설치 프로그램 다시 실행…") \
    X(TipAmdPortRerun,    "Fetches the latest dlssnr_on_amd_setup.exe and starts it in its own window: there it offers an update (U) or the removal (R) of DLSS-NR-on-AMD.", "获取最新的 dlssnr_on_amd_setup.exe 并在它自己的窗口中启动：在那里可以选择更新（U）或移除（R）DLSS-NR-on-AMD。", "最新の dlssnr_on_amd_setup.exe を取得して独自のウィンドウで起動します。そこで DLSS-NR-on-AMD の更新（U）または削除（R）を選べます。", "최신 dlssnr_on_amd_setup.exe를 받아 자체 창에서 실행합니다. 거기서 DLSS-NR-on-AMD의 업데이트(U) 또는 제거(R)를 고를 수 있습니다.") \
    X(AmdPortLoadedVersion, "DLSS-NR-on-AMD %s: loaded (%s)", "DLSS-NR-on-AMD %s：已加载（%s）", "DLSS-NR-on-AMD %s：読み込み済み（%s）", "DLSS-NR-on-AMD %s: 로드됨(%s)") \
    X(AmdPortLoadedCurrent, "DLSS-NR-on-AMD %s: loaded (%s) · up to date", "DLSS-NR-on-AMD %s：已加载（%s）· 已是最新", "DLSS-NR-on-AMD %s：読み込み済み（%s）· 最新です", "DLSS-NR-on-AMD %s: 로드됨(%s) · 최신 버전") \
    X(AmdPortNewer,       "DLSS-NR-on-AMD %s is out (%s); the installation is older.", "DLSS-NR-on-AMD %s 已发布（%s）；当前安装的是旧版本。", "DLSS-NR-on-AMD %s が公開されています（%s）。インストール済みのものは古い版です。", "DLSS-NR-on-AMD %s이(가) 나왔습니다(%s). 설치된 것은 이전 버전입니다.") \
    X(AmdPortVersionUnknown, "Installed release unknown; the latest is %s (%s). Running the installer again brings it to the latest.", "已安装的版本未知；最新为 %s（%s）。再次运行安装器即可更新到最新。", "インストール済みのバージョンは不明です。最新は %s（%s）です。インストーラーをもう一度実行すると最新になります。", "설치된 버전을 알 수 없습니다. 최신은 %s(%s)입니다. 설치 프로그램을 다시 실행하면 최신이 됩니다.") \
    X(TipAmdPortInstall,  "Downloads dlssnr_on_amd_setup.exe from the latest DLSS-NR-on-AMD release into the program folder and starts it. It is that project's own program under its own terms: it converts the nvngx_dlssnr.dll next to the executable into its weights file and installs its DLL. Running it again updates or removes the installation.", "从 DLSS-NR-on-AMD 的最新发布下载 dlssnr_on_amd_setup.exe 到程序目录并启动。它是该项目自己的程序、遵循其自身条款：它把可执行文件旁的 nvngx_dlssnr.dll 转换为自己的权重文件并安装其 DLL。再次运行可更新或移除。", "DLSS-NR-on-AMD の最新リリースから dlssnr_on_amd_setup.exe をプログラムフォルダーにダウンロードして起動します。これはそのプロジェクト自身のプログラムで、独自の条件に従います。実行ファイルの隣の nvngx_dlssnr.dll を自身の重みファイルに変換し、その DLL をインストールします。もう一度実行すると更新または削除できます。", "DLSS-NR-on-AMD 최신 릴리스에서 dlssnr_on_amd_setup.exe를 프로그램 폴더에 내려받아 실행합니다. 그 프로젝트 자체의 프로그램이며 자체 약관을 따릅니다. 실행 파일 옆의 nvngx_dlssnr.dll을 자체 가중치 파일로 변환하고 DLL을 설치합니다. 다시 실행하면 업데이트하거나 제거할 수 있습니다.") \
    X(AmdPortPage,        "Release page", "发布页", "リリースページ", "릴리스 페이지") \
    X(AmdPortChecking,    "Looking up the latest DLSS-NR-on-AMD release…", "正在查询 DLSS-NR-on-AMD 的最新发布…", "DLSS-NR-on-AMD の最新リリースを確認中…", "DLSS-NR-on-AMD 최신 릴리스 확인 중…") \
    X(AmdPortDownloading, "Downloading the DLSS-NR-on-AMD installer: %.1f / %.1f MB", "正在下载 DLSS-NR-on-AMD 安装器：%.1f / %.1f MB", "DLSS-NR-on-AMD のインストーラーをダウンロード中：%.1f / %.1f MB", "DLSS-NR-on-AMD 설치 프로그램 다운로드 중: %.1f / %.1f MB") \
    X(AmdPortInstalling,  "Installing DLSS-NR-on-AMD…", "正在安装 DLSS-NR-on-AMD…", "DLSS-NR-on-AMD をインストール中…", "DLSS-NR-on-AMD 설치 중…") \
    X(AmdPortInstallerRunning, "The DLSS-NR-on-AMD installer is running in its own window. Press Enter there to accept the proposed DLL name.", "DLSS-NR-on-AMD 安装器正在自己的窗口中运行。在那里按 Enter 接受它提议的 DLL 名称。", "DLSS-NR-on-AMD のインストーラーが別ウィンドウで実行中です。そこで Enter を押して提案された DLL 名を受け入れてください。", "DLSS-NR-on-AMD 설치 프로그램이 별도 창에서 실행 중입니다. 거기서 Enter를 눌러 제안된 DLL 이름을 수락하세요.") \
    X(AmdPortInstalled,   "DLSS-NR-on-AMD is installed. Restart the application to load it.", "DLSS-NR-on-AMD 已安装。重启程序以加载它。", "DLSS-NR-on-AMD をインストールしました。読み込むにはアプリを再起動してください。", "DLSS-NR-on-AMD가 설치되었습니다. 불러오려면 앱을 다시 시작하세요.") \
    X(AmdPortNotInstalled, "The DLSS-NR-on-AMD installer closed without installing (no weights file next to the executable).", "DLSS-NR-on-AMD 安装器已关闭但未完成安装（可执行文件旁没有权重文件）。", "DLSS-NR-on-AMD のインストーラーはインストールせずに終了しました（実行ファイルの隣に重みファイルがありません）。", "DLSS-NR-on-AMD 설치 프로그램이 설치하지 않고 종료되었습니다(실행 파일 옆에 가중치 파일이 없음).") \
    X(AmdPortFailed,      "DLSS-NR-on-AMD installer", "DLSS-NR-on-AMD 安装器", "DLSS-NR-on-AMD インストーラー", "DLSS-NR-on-AMD 설치 프로그램") \
    X(AmdPortRestartHint, "DLSS-NR-on-AMD is installed; it loads with the next start of the application.", "DLSS-NR-on-AMD 已安装；下次启动程序时加载。", "DLSS-NR-on-AMD をインストールしました。次回アプリ起動時に読み込まれます。", "DLSS-NR-on-AMD가 설치되었습니다. 다음 앱 시작 시 불러옵니다.") \
    X(RestartNow,         "Restart now", "立即重启", "今すぐ再起動", "지금 다시 시작") \
    X(AmdPortRequirements, "DLSS-NR-on-AMD needs Windows 11, a Radeon RX 7000 or RX 9000 card and Adrenalin 26.1.1 or newer; its own overlay (End key) holds its strengths.", "DLSS-NR-on-AMD 需要 Windows 11、Radeon RX 7000 或 RX 9000 显卡以及 Adrenalin 26.1.1 或更新版本；它自己的叠加层（End 键）中有其强度设置。", "DLSS-NR-on-AMD には Windows 11、Radeon RX 7000 または RX 9000、Adrenalin 26.1.1 以降が必要です。強度は独自のオーバーレイ（End キー）にあります。", "DLSS-NR-on-AMD에는 Windows 11, Radeon RX 7000 또는 RX 9000 카드, Adrenalin 26.1.1 이상이 필요합니다. 강도는 자체 오버레이(End 키)에 있습니다.") \
    X(AmdPortOffer,       "On a Radeon card the neural pass is done by DLSS-NR-on-AMD (Daniel Blanco), a separate program under its own licence: free for personal, non-commercial use, not to be redistributed or modified. The button downloads its installer from its own release page and starts it here; once it has installed, this application restarts by itself.", "在 Radeon 显卡上，神经渲染由 DLSS-NR-on-AMD（Daniel Blanco）完成。它是一个独立程序，遵循其自身许可：个人非商业用途免费，不得再分发或修改。按钮会从它自己的发布页下载安装器并在此启动；安装完成后本程序会自动重启。", "Radeon ではニューラルパスを DLSS-NR-on-AMD（Daniel Blanco）が行います。これは独自のライセンスに従う別のプログラムです。個人的・非商用の利用は無料、再配布や改変は不可です。ボタンを押すと、そのリリースページからインストーラーをダウンロードしてここで起動します。インストールが終わるとこのアプリは自動的に再起動します。", "Radeon 카드에서는 뉴럴 패스를 DLSS-NR-on-AMD(Daniel Blanco)가 수행합니다. 자체 라이선스를 따르는 별도의 프로그램으로, 개인·비상업 용도는 무료이며 재배포와 수정은 금지됩니다. 버튼을 누르면 그 릴리스 페이지에서 설치 프로그램을 내려받아 여기서 실행합니다. 설치가 끝나면 이 앱은 스스로 다시 시작합니다.") \
    X(AmdPortLicense,     "Licence", "许可", "ライセンス", "라이선스") \
    X(AmdWelcomeTitle,    "DLSS 5 on this Radeon card", "在这块 Radeon 显卡上使用 DLSS 5", "この Radeon カードで DLSS 5 を使う", "이 Radeon 카드에서 DLSS 5 사용") \
    X(AmdPortWeightsNotLoaded, "DLSS-NR-on-AMD is installed next to the executable, but its DLL did not attach to this process. Run its installer again; its log (dlssnr_on_amd.log in the program folder) says more.", "DLSS-NR-on-AMD 已安装在可执行文件旁，但其 DLL 没有挂接到本进程。请再次运行其安装器；程序目录中的 dlssnr_on_amd.log 有更多信息。", "DLSS-NR-on-AMD は実行ファイルの隣にインストールされていますが、その DLL はこのプロセスに取り付いていません。インストーラーをもう一度実行してください。プログラムフォルダーの dlssnr_on_amd.log に詳細があります。", "DLSS-NR-on-AMD가 실행 파일 옆에 설치되어 있지만 그 DLL이 이 프로세스에 붙지 않았습니다. 설치 프로그램을 다시 실행하세요. 프로그램 폴더의 dlssnr_on_amd.log에 자세한 내용이 있습니다.") \
    X(AmdPortRestartIn,   "DLSS-NR-on-AMD is installed; the application restarts in %d s to load it.", "DLSS-NR-on-AMD 已安装；程序将在 %d 秒后重启以加载它。", "DLSS-NR-on-AMD をインストールしました。%d 秒後にアプリを再起動して読み込みます。", "DLSS-NR-on-AMD가 설치되었습니다. %d초 뒤 앱을 다시 시작하여 불러옵니다.") \
    X(FsrNoEffect,        "The FSR host runs, but the picture comes back unchanged: DLSS-NR-on-AMD is not attached to this process, so no neural pass is applied.", "FSR 宿主在运行，但画面没有变化：DLSS-NR-on-AMD 未挂接到本进程，所以没有应用神经渲染。", "FSR ホストは動いていますが画像は変わっていません。DLSS-NR-on-AMD がこのプロセスに取り付いていないため、ニューラルパスは適用されていません。", "FSR 호스트는 동작하지만 화면이 그대로입니다. DLSS-NR-on-AMD가 이 프로세스에 붙어 있지 않아 뉴럴 패스가 적용되지 않았습니다.") \
    X(EditionAmd,         "Radeon edition", "Radeon 版", "Radeon 版", "Radeon 에디션") \
    X(EditionGeforce,     "GeForce edition", "GeForce 版", "GeForce 版", "GeForce 에디션") \
    X(EditionGetAmd,      "Get the Radeon edition…", "获取 Radeon 版…", "Radeon 版を入手…", "Radeon 에디션 받기…") \
    X(EditionGetGeforce,  "Get the GeForce edition…", "获取 GeForce 版…", "GeForce 版を入手…", "GeForce 에디션 받기…") \
    X(TipEditionSwitch,   "Downloads the other edition of this application from its release page and replaces the program files (the application closes and starts again). Settings and library stay.", "从发布页下载本程序的另一版本并替换程序文件（程序会关闭并重新启动）。设置与素材库保留。", "リリースページからこのアプリのもう一方の版をダウンロードしてプログラムファイルを置き換えます（アプリは終了して再起動します）。設定とライブラリは残ります。", "릴리스 페이지에서 이 앱의 다른 에디션을 내려받아 프로그램 파일을 바꿉니다(앱이 종료된 뒤 다시 시작). 설정과 라이브러리는 유지됩니다.") \
    X(EditionHintAmdOnGeforce, "This is the Radeon edition. On a GeForce card the GeForce edition carries the runtime builds for every RTX generation.", "这是 Radeon 版。在 GeForce 显卡上，GeForce 版自带各代 RTX 的运行库。", "これは Radeon 版です。GeForce では、GeForce 版が各 RTX 世代向けのランタイムを同梱しています。", "이 프로그램은 Radeon 에디션입니다. GeForce 카드에서는 GeForce 에디션이 모든 RTX 세대용 런타임을 포함합니다.") \
    X(EditionHintGeforceOnAmd, "This is the GeForce edition. On a Radeon card the Radeon edition runs the FSR host that DLSS-NR-on-AMD attaches to.", "这是 GeForce 版。在 Radeon 显卡上，Radeon 版运行供 DLSS-NR-on-AMD 挂接的 FSR 宿主。", "これは GeForce 版です。Radeon では、Radeon 版が DLSS-NR-on-AMD の取り付く FSR ホストを動かします。", "이 프로그램은 GeForce 에디션입니다. Radeon 카드에서는 Radeon 에디션이 DLSS-NR-on-AMD가 붙는 FSR 호스트를 실행합니다.") \
    X(UpdateEditionTitle, "Edition switch", "切换版本", "版の切り替え", "에디션 전환") \
    X(UpdateEditionFmt,   "%s %s is available (this is the %s %s).", "%s %s 可用（当前为 %s %s）。", "%s %s が利用できます（現在は %s %s）。", "%s %s을(를) 사용할 수 있습니다(현재 %s %s).") \
    X(EditionNotFound,    "No %s of version %s or newer was found on the release page.", "发布页上没有找到 %s（版本 %s 或更新）。", "リリースページに %s（バージョン %s 以降）は見つかりませんでした。", "릴리스 페이지에서 %s(버전 %s 이상)을(를) 찾을 수 없습니다.") \
    ...
    X(MotionFsr,          "FSR optical flow (compute, any card)", "FSR 光流（计算着色器，任意显卡）", "FSR オプティカルフロー（コンピュート・全カード）", "FSR 옵티컬 플로우(컴퓨트, 모든 카드)") \
    ...
    X(FlowBidirectional,  "Bidirectional consistency check", "双向一致性检查", "双方向一貫性チェック", "양방향 일관성 검사") \
    X(TipFlowBidirectional, "Searches the finest level once more with the two pictures the other way round and lowers the confidence where the two directions disagree (occlusions, repeating patterns). Recommended; it costs about a third of the flow time.", "在最细的一层以相反方向再搜索一次，两个方向不一致处（遮挡、重复纹理）降低置信度。推荐开启，约增加三分之一的光流耗时。", "最も細かいレベルを逆方向にもう一度探索し、両方向が一致しない箇所（オクルージョン、繰り返し模様）の信頼度を下げます。推奨。フロー時間が約 3 分の 1 増えます。", "가장 세밀한 레벨을 반대 방향으로 한 번 더 탐색하여 두 방향이 일치하지 않는 곳(가림, 반복 패턴)의 신뢰도를 낮춥니다. 권장하며 플로우 시간이 약 3분의 1 늘어납니다.") \
    X(FlowLevels,         "levels", "层", "レベル", "레벨") \
    X(TipFlowRadius,      "Full-search radius at the coarsest level of the pyramid. One pixel there stands for as many pixels of the picture as the level count doubles: radius 7 over six levels follows motion of about 224 pixels.", "金字塔最粗一层上的全搜索半径。该层的 1 像素相当于画面的 2^(层数-1) 像素：6 层时半径 7 可跟踪约 224 像素的运动。", "ピラミッドの最も粗いレベルでの全探索半径。そのレベルの 1 ピクセルは画面の 2^(レベル数-1) ピクセルに相当し、6 レベルで半径 7 なら約 224 ピクセルの動きまで追跡できます。", "피라미드의 가장 거친 레벨에서의 전체 탐색 반경. 그 레벨의 1픽셀은 화면의 2^(레벨 수-1) 픽셀에 해당하여, 6레벨에서 반경 7이면 약 224픽셀의 움직임까지 추적합니다.") \
    ...
    X(DlaaTooLarge,       "DLSS takes outputs up to 8192 pixels a side: the anti-aliasing is off for this output size.", "DLSS 最大支持单边 8192 像素的输出：此输出尺寸下抗锯齿不生效。", "DLSS が受け付ける出力は一辺 8192 ピクセルまでです。この出力サイズではアンチエイリアスは無効です。", "DLSS는 한 변 8192픽셀까지의 출력만 받습니다. 이 출력 크기에서는 안티앨리어싱이 꺼집니다.") \
    X(DlaaPreset,         "DLSS preset", "DLSS 预设", "DLSS プリセット", "DLSS 프리셋") \
    X(CaptureFolder,      "Folder", "保存文件夹", "保存先", "저장 폴더") \
    X(CaptureName,        "File name", "文件名", "ファイル名", "파일 이름") \
    X(OutputName,         "Output file name", "输出文件名", "出力ファイル名", "출력 파일 이름") \
    X(TipFileName,        "The file name, from a template. {name} is the source file's name without its extension (VRChat for a live capture), {date} the day (2026-09-14), {time} the time of day (12-34-56.123), {size} the saved picture's size (1920x1080), {width} and {height} its sides, {insize}, {inwidth} and {inheight} the source's. Everything else is kept as typed; characters a file name cannot hold become _. A name already taken gets _2, _3, … Empty = the default shown in the box.", "文件名模板。{name} 为源文件名（不含扩展名，实时拍照时为 VRChat），{date} 为日期（2026-09-14），{time} 为时刻（12-34-56.123），{size} 为保存图片的尺寸（1920x1080），{width}、{height} 为其宽高，{insize}、{inwidth}、{inheight} 为源画面的尺寸与宽高。其余文字原样保留，文件名不允许的字符会换成 _；重名时自动加 _2、_3……留空即使用框内显示的默认值。", "ファイル名のテンプレートです。{name} は元ファイル名（拡張子なし、ライブ撮影では VRChat）、{date} は日付（2026-09-14）、{time} は時刻（12-34-56.123）、{size} は保存画像のサイズ（1920x1080）、{width}・{height} はその幅と高さ、{insize}・{inwidth}・{inheight} は元画像のものです。その他の文字はそのまま使われ、ファイル名に使えない文字は _ に置き換えられます。同名がある場合は _2、_3… が付きます。空欄にすると枠内に示す既定値になります。", "파일 이름 템플릿입니다. {name}은 원본 파일 이름(확장자 제외, 실시간 촬영에서는 VRChat), {date}는 날짜(2026-09-14), {time}은 시각(12-34-56.123), {size}는 저장되는 사진의 크기(1920x1080), {width}·{height}는 그 너비와 높이, {insize}·{inwidth}·{inheight}는 원본의 값입니다. 나머지 글자는 그대로 쓰이고, 파일 이름에 쓸 수 없는 문자는 _로 바뀝니다. 같은 이름이 있으면 _2, _3…이 붙습니다. 비워 두면 칸에 표시된 기본값을 씁니다.") \
    ...
    X(NvofNoEngine,       "Not on this card (NVIDIA cards only); the FSR optical flow is used", "此显卡没有该引擎（仅 NVIDIA 显卡有），已改用 FSR 光流", "このカードにはありません（NVIDIA カードのみ）。FSR オプティカルフローを使用中", "이 카드에는 없습니다(NVIDIA 카드 전용). FSR 옵티컬 플로우 사용 중") \
    ...
    X(TipToolRowShow,     "Bring the toolbar back", "展开工具条", "ツールバーを戻します", "도구 모음을 다시 펼칩니다") \
    ...
    X(SecMcp,             "MCP", "MCP", "MCP", "MCP") \
    X(McpHint,            "MCP clients (Claude, Cursor, chat bots and others) can open files, change the settings, process and save pictures and videos and look at the preview, through the same actions as this window. Other computers may send jobs with a key: they wait in a queue, and each caller is told its place and when its turn comes.", "MCP 客户端（Claude、Cursor、聊天机器人等）可以像在此窗口中一样打开文件、修改设置、处理并保存图片和视频、查看预览。其他电脑可凭密钥提交任务：任务排队等候，每个调用方都会得知自己的位置和轮到的时间。", "MCP クライアント（Claude、Cursor、チャットボットなど）が、このウィンドウと同じ操作でファイルを開き、設定を変え、画像や動画を処理して保存し、プレビューを見られます。他の PC は鍵を使ってジョブを送れます。ジョブはキューで待ち、各呼び出し元に順番と開始時刻が伝えられます。", "MCP 클라이언트(Claude, Cursor, 챗봇 등)가 이 창과 같은 동작으로 파일을 열고, 설정을 바꾸고, 사진과 영상을 처리·저장하고, 미리보기를 볼 수 있습니다. 다른 PC는 키로 작업을 보낼 수 있으며, 작업은 대기열에서 기다리고 각 호출자는 자기 순서와 차례를 안내받습니다.") \
    X(McpEnable,          "Run the MCP server", "运行 MCP 服务器", "MCP サーバーを実行", "MCP 서버 실행") \
    X(TipMcp,             "The server runs whenever the program does. A client connected through the bridge (VRChatDLSS5Cam.exe --mcp) starts the program and its server by itself, whatever this switch says.", "程序运行时服务器就一直运行。通过桥接方式（VRChatDLSS5Cam.exe --mcp）接入的客户端会自动启动程序和服务器，与此开关无关。", "プログラムの実行中はサーバーも動き続けます。ブリッジ（VRChatDLSS5Cam.exe --mcp）経由のクライアントは、このスイッチに関わらずプログラムとサーバーを自分で起動します。", "프로그램이 실행되는 동안 서버도 계속 실행됩니다. 브리지(VRChatDLSS5Cam.exe --mcp)로 접속한 클라이언트는 이 스위치와 관계없이 프로그램과 서버를 스스로 시작합니다.") \
    ...
    X(TipMcpReadOnly,     "Clients may look (status, settings, log, preview) but not change anything.", "客户端只能查看（状态、设置、日志、预览），不能做任何修改。", "クライアントは見るだけ（状態、設定、ログ、プレビュー）で、何も変更できません。", "클라이언트는 보기만 할 수 있고(상태, 설정, 로그, 미리보기) 아무것도 바꿀 수 없습니다.") \
    X(McpOn,              "Listening", "监听中", "待ち受け中", "수신 대기 중") \
    X(McpOff,             "Off", "已关闭", "オフ", "꺼짐") \
    X(McpSessionNote,     "Started for this session by a client", "由客户端为本次会话启动", "クライアントがこのセッションのために起動しました", "클라이언트가 이 세션을 위해 시작함") \
