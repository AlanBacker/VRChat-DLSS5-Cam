...
#define VDC_STRING_LIST(X) \
    ...
    X(TipIntensity,       "Overall strength of the neural rendering (1 = default, 0 = off). The runtime itself stops at 1; between 1 and 2 the composite pass amplifies the whole change the network made, which can also exaggerate artifacts.", "神经渲染的整体强度（1 = 默认，0 = 关闭）。运行库本身最高只到 1；1–2 之间由合成阶段放大网络带来的全部变化，瑕疵也可能一并被放大。", "ニューラルレンダリングの全体強度（1 = 既定、0 = オフ）。ランタイム自体は 1 が上限で、1–2 の範囲では合成段階でネットワークが加えた変化全体を増幅します（アーティファクトも強調されることがあります）。", "뉴럴 렌더링의 전체 강도(1 = 기본, 0 = 끔). 런타임 자체는 1까지이며, 1–2 구간에서는 합성 단계에서 네트워크가 만든 변화 전체를 증폭합니다(아티팩트도 커질 수 있음).") \
    X(TipGlobalTone,      "How much the model may change global exposure and colour. Above 1 the composite pass amplifies the global tone curve of the network's change (brightness and colour as a function of the original lightness).", "允许模型改变全局曝光与色彩的程度。大于 1 时，合成阶段放大网络变化中的全局色调曲线（随原图明度变化的亮度与色彩）。", "モデルが全体の露出と色をどれだけ変えられるか。1 を超えると、合成段階でネットワークの変化のうちグローバルトーンカーブ（元の明るさに応じた明るさと色）を増幅します。", "모델이 전체 노출과 색을 얼마나 바꿀 수 있는지. 1을 넘으면 합성 단계에서 네트워크 변화 중 전역 톤 커브(원본 밝기에 따른 밝기와 색)를 증폭합니다.") \
    X(TipLocalTone,       "Local contrast and lighting adjustments. Above 1 the composite pass amplifies the local, low-frequency part of the network's change (dodge and burn, soft lighting).", "局部对比度与光照调整。大于 1 时，合成阶段放大网络变化中的局部低频部分（局部提亮压暗、柔和光照）。", "ローカルコントラストとライティングの調整。1 を超えると、合成段階でネットワークの変化のうちローカルな低周波成分（覆い焼き・焼き込み、柔らかな照明）を増幅します。", "로컬 대비와 조명 조정. 1을 넘으면 합성 단계에서 네트워크 변화 중 지역 저주파 성분(닷지와 번, 부드러운 조명)을 증폭합니다.") \
    X(TipLocalStructure,  "Fine detail and texture enhancement. Above 1 the composite pass amplifies the fine-detail part of the network's change.", "细节与纹理增强。大于 1 时，合成阶段放大网络变化中的细节部分。", "ディテールとテクスチャの強調。1 を超えると、合成段階でネットワークの変化のうち細部の成分を増幅します。", "세부 묘사와 질감 강조. 1을 넘으면 합성 단계에서 네트워크 변화 중 세부 성분을 증폭합니다.") \
    X(TipSkinStructure,   "Detail on skin-like surfaces. \"Runtime default\" leaves it to the model. Above 1 the composite pass amplifies the fine-detail part of the change on skin-coloured areas only.", "皮肤类表面的细节。“使用默认值”由模型自行决定。大于 1 时，合成阶段只在肤色区域放大变化中的细节部分。", "肌のような表面のディテール。「既定値を使用」でモデルに任せます。1 を超えると、合成段階で肌色の領域に限って変化のうち細部の成分を増幅します。", "피부 같은 표면의 디테일. \"기본값 사용\"이면 모델에 맡깁니다. 1을 넘으면 합성 단계에서 피부색 영역에서만 변화 중 세부 성분을 증폭합니다.") \
