...
#define VDC_STRING_LIST(X) \
    ...
    X(AmdPortMissing,     "On a Radeon card the neural pass is done by DLSS-NR-on-AMD, a separate program that attaches to this FSR host. Install it with the button below: its installer is downloaded from its own release page and started here; accept the DLL name it proposes (Enter), then restart this application.", "Radeon 显卡上神经渲染由 DLSS-NR-on-AMD 完成，它是一个独立程序，会挂接到这里的 FSR 宿主。用下方按钮安装：其安装器会从它自己的发布页下载并在此启动；接受它提议的 DLL 名称（按 Enter），然后重启本程序。", "Radeon ではニューラルパスを DLSS-NR-on-AMD が行います。これは別のプログラムで、この FSR ホストに取り付きます。下のボタンでインストールしてください。そのインストーラーは自身のリリースページからダウンロードされ、ここで起動します。提案された DLL 名を受け入れ（Enter）、その後このアプリを再起動してください。", "Radeon 카드에서는 뉴럴 패스를 DLSS-NR-on-AMD가 수행합니다. 이는 별도의 프로그램으로 이 FSR 호스트에 붙습니다. 아래 버튼으로 설치하세요. 설치 프로그램은 자체 릴리스 페이지에서 내려받아 여기서 실행됩니다. 제안된 DLL 이름을 수락(Enter)한 뒤 이 앱을 다시 시작하세요.") \
    X(AmdPortInstall,     "Install DLSS-NR-on-AMD…", "安装 DLSS-NR-on-AMD…", "DLSS-NR-on-AMD をインストール…", "DLSS-NR-on-AMD 설치…") \
    X(AmdPortUpdate,      "Update DLSS-NR-on-AMD…", "更新 DLSS-NR-on-AMD…", "DLSS-NR-on-AMD を更新…", "DLSS-NR-on-AMD 업데이트…") \
    ...
    X(AmdPortLatest,      "Latest DLSS-NR-on-AMD release: %s (%s)", "DLSS-NR-on-AMD 最新发布：%s（%s）", "DLSS-NR-on-AMD の最新リリース：%s（%s）", "DLSS-NR-on-AMD 최신 릴리스: %s(%s)") \
