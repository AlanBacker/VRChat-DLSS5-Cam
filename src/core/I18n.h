...
#define VDC_STRING_LIST(X) \
    ...
    X(RuntimeMissing,     "nvngx_dlssnr.dll was not found. The runtime belongs next to the executable, or its path can be set below.", "未找到 nvngx_dlssnr.dll。运行库应放在程序目录下，也可以在下方指定路径。", "nvngx_dlssnr.dll が見つかりません。ランタイムは実行ファイルと同じフォルダに置くか、下でパスを指定できます。", "nvngx_dlssnr.dll을 찾을 수 없습니다. 런타임은 실행 파일 옆에 두거나 아래에서 경로를 지정할 수 있습니다.") \
    X(RuntimeVariants,    "Builds for several architectures can sit side by side: runtimes\\blackwell\\ for RTX 50, runtimes\\universal\\ for RTX 40/30/20, runtimes\\other\\ for a card from another vendor. The build matching this adapter is chosen automatically.", "可以同时放置多个架构的运行库：runtimes\\blackwell\\ 对应 RTX 50，runtimes\\universal\\ 对应 RTX 40/30/20，runtimes\\other\\ 对应其他厂商的显卡。程序会自动选择与当前显卡匹配的版本。", "複数のアーキテクチャ向けビルドを併置できます。RTX 50 は runtimes\\blackwell\\、RTX 40/30/20 は runtimes\\universal\\、他社製カードは runtimes\\other\\ に置くと、このアダプターに合うビルドが自動的に選ばれます。", "여러 아키텍처용 빌드를 함께 둘 수 있습니다. RTX 50은 runtimes\\blackwell\\, RTX 40/30/20은 runtimes\\universal\\, 다른 제조사 카드는 runtimes\\other\\에 두면 이 어댑터에 맞는 빌드가 자동으로 선택됩니다.") \
