...
#define VDC_STRING_LIST(X) \
    ...
    X(NrOtherVendorHint,  "The bundled runtime builds are NVIDIA's and start only on GeForce RTX cards. The application works as a viewer and recorder here; the neural pass needs a runtime adapted for this vendor under runtimes\\other\\.", "附带的运行库是 NVIDIA 的，只能在 GeForce RTX 显卡上启动。程序在此可作为查看器和录制工具使用；神经渲染需要为该厂商适配的运行库，放在 runtimes\\other\\ 下。", "同梱のランタイムビルドは NVIDIA のもので、GeForce RTX でのみ起動します。ここではビューアーおよび録画ツールとして動作します。ニューラルパスにはこのベンダー向けに適合させたランタイムを runtimes\\other\\ に置く必要があります。", "내장 런타임 빌드는 NVIDIA의 것이며 GeForce RTX 카드에서만 시작됩니다. 여기서는 뷰어와 녹화 도구로 동작합니다. 뉴럴 패스에는 이 제조사용으로 적합화된 런타임을 runtimes\\other\\에 두어야 합니다.") \
