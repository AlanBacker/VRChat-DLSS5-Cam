...
const std::vector<McpTool> kTools = {
    ...
    { "vdc_submit",
      ...
      "with name, path (a file on the server's own disk: admin only), or job_id (the input of an earlier job of yours: the same file again with another look). The look: preset (the name of a saved preset, or default for the program's default look) and "
      ...
      R"json({"type":"object","properties":{"upload_id":{"type":"string"},"url":{"type":"string"},"data":{"type":"string","description":"The file's bytes as base64"},"name":{"type":"string","description":"The file name of data or url (its extension says the format)"},"path":{"type":"string","description":"A file on the server's computer (admin only)"},"job_id":{"type":"string","description":"An earlier job of yours whose input is processed again"},"preset":{"type":"string"},"settings":{"type":"object","additionalProperties":true},"in":{"type":"number"},"out":{"type":"number"},"label":{"type":"string"},"client_ref":{"type":"string"},"wait":{"type":"boolean","default":false},"timeout":{"type":"number","default":30,"maximum":60}},"additionalProperties":false})json", true, true, false },
    { "vdc_upload",
