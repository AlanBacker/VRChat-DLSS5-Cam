...
void McpCall::Image(const std::vector<uint8_t>& png) {
    content.Push(Json::Obj().Set("type", "image").Set("data", McpServer::Base64(png.data(), png.size())).Set("mimeType", "image/png"));
}
...
const char* kInstructions =
    ...
    "this computer. Sizes are pixels, times are seconds, strengths are 0..2 with 1 as the runtime's own value. In "
    "read-only mode only the looking tools answer.";
...
const std::vector<McpTool> kTools = {
    { "get_status",
      ...
      R"json({"type":"object","properties":{},"additionalProperties":false})json", false },
    { "describe_settings",
    ...
    { "get_settings",
      ...
      R"json({"type":"object","properties":{"keys":{"type":"array","items":{"type":"string"},"description":"Only these keys; all when omitted"}},"additionalProperties":false})json", false },
    { "set_settings",
      ...
      R"json({"type":"object","properties":{"settings":{"type":"object","description":"Keys and their new values, for example {\"nrIntensity\": 1.5, \"nrStyle\": 2, \"compareMode\": 0}","additionalProperties":true}},"required":["settings"],"additionalProperties":false})json", true },
    { "reset_settings",
      "Every setting back to its default (the window placement, language and the opened file stay), like the Reset button.",
      R"json({"type":"object","properties":{},"additionalProperties":false})json", true },
    { "open",
    ...
    { "open_live",
      ...
      R"json({"type":"object","properties":{"sender":{"type":"string","description":"Spout sender name; empty = automatic (prefers VRCSender1)"}},"additionalProperties":false})json", true },
    { "list_senders",
    ...
    { "capture",
      ...
      R"json({"type":"object","properties":{"wait":{"type":"boolean","default":true},"timeout":{"type":"number","default":180}},"additionalProperties":false})json", true },
    { "cancel",
    ...
    { "wait",
      ...
      R"json({"type":"object","properties":{"for":{"type":"string","enum":["idle","loaded","converged","display"],"default":"idle"},"timeout":{"type":"number","default":60}},"additionalProperties":false})json", false },
    { "undo", "Undoes the last change (Ctrl+Z), or several.",
      R"json({"type":"object","properties":{"steps":{"type":"integer","default":1,"minimum":1}},"additionalProperties":false})json", true },
    ...
    { "history", "The undo history: every recorded state with a label, and which one is current; with index, the program goes to that state (the history panel's click).",
      R"json({"type":"object","properties":{"index":{"type":"integer","minimum":0,"description":"Go to this state of the list, 0 = the oldest"}},"additionalProperties":false})json", true },
    { "presets",
      ...
      R"json({"type":"object","properties":{"action":{"type":"string","enum":["list","apply","save","delete","rename"]},"name":{"type":"string"},"new_name":{"type":"string","description":"rename: the new name"}},"required":["action"],"additionalProperties":false})json", true },
    { "get_log",
      "The last lines of the program's log (what the runtime, the passes, the files and the errors reported).",
      R"json({"type":"object","properties":{"lines":{"type":"integer","default":50,"minimum":1,"maximum":500},"level":{"type":"string","enum":["info","warn","error"],"description":"Only entries of this level and above"},"contains":{"type":"string","description":"Only lines with this text"}},"additionalProperties":false})json", false },
    { "window",
    ...
    { "reload",
      ...
      "of the passes.",
      R"json({"type":"object","properties":{"what":{"type":"string","enum":["runtime","depth","senders","history"]}},"required":["what"],"additionalProperties":false})json", true },
};
...
std::string QueryValue(const std::string& query, const char* key) {
    ...
    while (pos < query.size()) {
        ...
        if (eq != std::string::npos && part.substr(0, eq) == key) return part.substr(eq + 1);
        pos = amp + 1;
    }
    return std::string();
}
...
// A plain HTTP exchange with a server on this computer (the bridge, the wake-up check).
bool HttpLocal(int port, const std::string& method, const std::string& path, const std::string& body, const std::string& token,
               int& status, std::string& response, double timeoutSeconds) {
    status = 0;
    response.clear();
    ...
    size_t sent = 0;
    while (sent < req.size()) {
        const int n = send(s, req.data() + sent, (int)std::min<size_t>(req.size() - sent, 1 << 16), 0);
...
void McpServer::SetToken(const std::string& token) { std::lock_guard<std::mutex> lock(m_mutex); m_token = token; }
...
void McpServer::NoteCall(const std::string& tool) {
    std::lock_guard<std::mutex> lock(m_mutex);
    ++m_calls;
    m_lastTool = tool;
...
bool McpServer::Start(int port, Dispatch dispatch) {
    Stop();
    ...
    Log::Info("MCP: listening at %s", Url(port).c_str());
    return true;
}
...
void McpServer::ListenMain() {
    while (!m_quit.load()) {
        ...
        if (from.sin_addr.s_addr != htonl(INADDR_LOOPBACK)) { closesocket(c); continue; }   // never anything but this computer
        ...
        m_live.fetch_add(1);
        std::thread([this, c] { Connection((uintptr_t)c); m_live.fetch_sub(1); }).detach();
    }
}
...
bool McpServer::ReadRequest(uintptr_t socket, Request& r) {
    ...
    while (pos < head.size()) {
        ...
        if (name == "content-length") contentLength = (size_t)atoll(value.c_str());
    ...
    r.body = data.substr(hdrEnd + 4);
    while (r.body.size() < contentLength) {
        const int n = recv(s, buf, (int)std::min<size_t>(sizeof(buf), contentLength - r.body.size()), 0);
        if (n <= 0) return false;
        r.body.append(buf, (size_t)n);
    }
    r.body.resize(contentLength);
    return true;
}

void McpServer::Connection(uintptr_t socket) {
    SOCKET s = (SOCKET)socket;
    ...
    while (!m_quit.load()) {
        ...
        Response out;
        Handle(r, out);
        const char* reason = out.status == 200 ? "OK" : out.status == 202 ? "Accepted" : out.status == 204 ? "No Content" :
                             out.status == 400 ? "Bad Request" : out.status == 401 ? "Unauthorized" : out.status == 403 ? "Forbidden" :
                             out.status == 404 ? "Not Found" : out.status == 405 ? "Method Not Allowed" : out.status == 406 ? "Not Acceptable" : "Error";
        std::string resp = "HTTP/1.1 " + std::to_string(out.status) + " " + reason + "\r\n";
        if (!out.body.empty() || (out.status != 202 && out.status != 204)) resp += "Content-Type: " + out.type + "\r\n";
        resp += "Content-Length: " + std::to_string(out.body.size()) + "\r\n";
        ...
        while (sent < resp.size()) {
            const int n = send(s, resp.data() + sent, (int)std::min<size_t>(resp.size() - sent, 1 << 16), 0);
...
void McpServer::Handle(const Request& r, Response& out) {
    ...
    if (!LocalOrigin(r.origin)) { out.status = 403; out.body = R"({"error":"origin not allowed"})"; return; }
    std::string token;
    { std::lock_guard<std::mutex> lock(m_mutex); token = m_token; }
    if (!token.empty()) {
        const std::string given = r.auth.size() > 7 && Lower(r.auth.substr(0, 7)) == "bearer " ? Trim(r.auth.substr(7)) : QueryValue(r.query, "token");
        if (given != token) { out.status = 401; out.extraHeaders = "WWW-Authenticate: Bearer\r\n"; out.body = R"({"error":"a bearer token is required"})"; return; }
    }
    if (r.path == "/mcp" || r.path == "/") {
        if (r.method == "GET" && r.path == "/") { out.type = "text/html; charset=utf-8"; out.body = InfoPage(); return; }
        if (r.method == "GET") { out.status = 405; out.extraHeaders = "Allow: POST, DELETE, OPTIONS\r\n"; out.body = R"({"error":"this server has no event stream; POST JSON-RPC messages"})"; return; }
        ...
        if (msg.type == Json::Array) {
            ...
            for (const Json& m : msg.arr) { bool none = false; Json reply = HandleRpc(m, none); if (!none) replies.Push(std::move(reply)); }
            if (replies.arr.empty()) { out.status = 202; return; }
            out.body = replies.Dump();
            return;
        }
        bool none = false;
        Json reply = HandleRpc(msg, none);
    ...
    if (r.method == "GET" && (r.path == "/preview.png" || r.path == "/window.png")) {
        ...
        Json err;
        Json result = CallTool("preview", args, &err);
        std::string png;
        ...
        for (char ch : png) {
            ...
            else if (ch == '+') v = 62; else if (ch == '/') v = 63; else break;
            acc = (acc << 6) | (unsigned)v; bits += 6;
            if (bits >= 8) { bits -= 8; bytes += (char)((acc >> bits) & 0xFF); }
        }
        out.type = "image/png";
        out.body = std::move(bytes);
        return;
    }
    if (r.method == "GET" && (r.path == "/status.json" || r.path == "/settings.json" || r.path == "/library.json" || r.path == "/log.txt")) {
        const char* tool = r.path == "/status.json" ? "get_status" : r.path == "/settings.json" ? "get_settings" : r.path == "/library.json" ? "library" : "get_log";
        Json args = Json::Obj();
        if (r.path == "/library.json") args.Set("action", "list");
        if (r.path == "/log.txt") { args.Set("lines", 200); out.type = "text/plain; charset=utf-8"; }
        Json err;
        Json result = CallTool(tool, args, &err);
        std::string text;
    ...
    out.status = 404;
    out.body = R"({"error":"not found"})";
}

Json McpServer::HandleRpc(const Json& msg, bool& noReply) {
    noReply = false;
    ...
    if (method == "initialize") {
        ...
        Json result = Json::Obj()
            ...
            .Set("instructions", kInstructions);
    ...
    if (method == "tools/call") {
        ...
        Json err;
        Json result = CallTool(name, ap ? *ap : Json::Obj(), &err);
    ...
    if (method == "resources/read") {
        Json err;
        Json result = ReadResource(params.Str("uri"), &err);
        if (err.type != Json::Null) return Json::Obj().Set("jsonrpc", "2.0").Set("id", id).Set("error", err);
        return RpcResult(id, result);
    }
...
Json McpServer::CallTool(const std::string& name, const Json& args, Json* rpcError) {
    ...
    if (m_readOnly.load() && !looking) {
        call->Fail("The MCP server is in read-only mode: " + name + " would change something. The user can switch it off in the sidebar (AI assistant section).");
    } else if (!m_dispatch) {
...
Json McpServer::ReadResource(const std::string& uri, Json* rpcError) {
    std::string tool;
    Json args = Json::Obj();
...
std::string McpServer::InfoPage() {
    Status st = Get();
    std::string h = "<!doctype html><html><head><meta charset=\"utf-8\"><title>VRChat DLSS5 Cam MCP server</title>"
                    "<style>body{font-family:Segoe UI,sans-serif;max-width:900px;margin:2em auto;padding:0 1em;color:#222}code,pre{background:#f3f3f3;padding:2px 5px;border-radius:4px}"
                    "pre{padding:10px;overflow:auto}table{border-collapse:collapse}td,th{border:1px solid #ddd;padding:6px 10px;text-align:left;vertical-align:top}img{max-width:100%}</style></head><body>";
    h += "<h1>VRChat DLSS5 Cam " APP_VERSION_STRING " &middot; MCP server</h1>";
    h += "<p>An MCP client (Claude Desktop, Claude Code, Cursor, ...) reaches this program at <code>" + HtmlEscape(Url(st.port)) +
         "</code> (Streamable HTTP, JSON-RPC 2.0 by POST), or through the stdio bridge <code>VRChatDLSS5Cam.exe --mcp</code>. " +
         StrPrintf("%u calls so far.", st.calls) + "</p>";
    h += "<p><a href=\"/preview.png\">preview.png</a> &middot; <a href=\"/window.png\">window.png</a> &middot; <a href=\"/status.json\">status.json</a> &middot; "
         "<a href=\"/settings.json\">settings.json</a> &middot; <a href=\"/library.json\">library.json</a> &middot; <a href=\"/log.txt\">log.txt</a></p>";
    h += "<h2>Tools</h2><table><tr><th>Tool</th><th>What it does</th></tr>";
    for (const McpTool& t : kTools) h += "<tr><td><code>" + std::string(t.name) + "</code>" + (t.mutating ? "" : " <small>(read)</small>") + "</td><td>" + HtmlEscape(t.description) + "</td></tr>";
...
void BridgeLog(const std::wstring& dir, const std::string& text) {
    if (dir.empty()) return;
    FILE* f = nullptr;
    if (_wfopen_s(&f, JoinPath(dir, L"mcp-bridge.txt").c_str(), L"ab") != 0 || !f) return;
    SYSTEMTIME t{};
    GetLocalTime(&t);
...
bool ServerAnswers(int port, const std::string& token) {
    int status = 0;
    std::string body;
    return HttpLocal(port, "POST", "/mcp", R"({"jsonrpc":"2.0","id":0,"method":"ping"})", token, status, body, 2.0) && status == 200;
}
...
int McpServer::BridgeMain(const std::wstring& exePath, int port, const std::wstring& dataDir, const std::wstring& passThrough) {
    ...
    // The port and the token come from the program's own settings file unless the command line names the port.
    std::string token;
    int filePort = 0;
    {
        FILE* f = nullptr;
        if (_wfopen_s(&f, JoinPath(logDir, L"settings.ini").c_str(), L"rb") == 0 && f) {
            ...
            while (fgets(line, sizeof(line), f)) {
                ...
                else if (l.rfind("mcpToken=", 0) == 0) token = l.substr(9);
            }
            fclose(f);
        }
    }
    if (port <= 0) port = filePort > 0 ? filePort : DefaultPort();
    ...
    BridgeLog(logDir, StrPrintf("bridge start, port %d", port));
    ...
    auto ensureServer = [&]() -> bool {
        if (ServerAnswers(port, token)) return true;
        HWND w = FindWindowW(WindowClass(), nullptr);
        if (w) {
            PostMessageW(w, kWakeMessage, (WPARAM)port, 0);
            BridgeLog(logDir, "asked the running program to start its server");
            for (int i = 0; i < 50; ++i) { Sleep(100); if (ServerAnswers(port, token)) return true; }
        }
        std::wstring cmd = L"\"" + exePath + L"\" --mcp-port " + std::to_wstring(port);
        ...
        for (int i = 0; i < 600; ++i) { Sleep(200); if (ServerAnswers(port, token)) return true; }
        ...
        while ((nl = pending.find('\n')) != std::string::npos) {
            ...
            if (!ensureServer()) {
                if (!notification) {
                    ...
                    WriteAll(out, RpcError(id, -32000, "VRChat DLSS5 Cam is not running and could not be started").Dump() + "\n");
                }
                continue;
            }
            int status = 0;
            std::string body;
            const bool ok = HttpLocal(port, "POST", "/mcp", line, token, status, body, 900.0);
            if (!ok) {
