...
bool Pipeline::Rebuild(GpuContext& gpu, const Config& cfg) {
    ...
    m_nrInW = m_inW; m_nrInH = m_inH; m_nrOutW = m_outW; m_nrOutH = m_outH;
...
// The neural pass textures follow the input size (and the output size when the pass itself upscales); a size change
// re-creates them and releases the feature, RunNeural creates it again. The flow and depth chains keep their history.
bool Pipeline::EnsureNeuralTextures(GpuContext& gpu, UINT inW, UINT inH, UINT outW, UINT outH) {
    if (inW == m_nrInW && inH == m_nrInH && outW == m_nrOutW && outH == m_nrOutH) return true;
    gpu.WaitIdle();
    ...
    m_nrInExposed = false;
    m_nrInW = inW; m_nrInH = inH; m_nrOutW = outW; m_nrOutH = outH;
    if (ok) Log::Info("Neural pass textures: %ux%u -> %ux%u", inW, inH, outW, outH);
...
Pipeline::Tex& Pipeline::PrepareNeuralInput(GpuContext& gpu, ID3D12GraphicsCommandList* cmd, const Settings& s, Tex& base) {
    ...
    if (!m_nrIn.Valid() || !exposed) return base;
...
void Pipeline::RunComposite(GpuContext& gpu, ID3D12GraphicsCommandList* cmd, const Settings& s, Tex& processed, Tex* neuralBase,
    ...
    if (!bypass && neuralBase && neuralInput &&
        (neuralInput != neuralBase || std::fabs(s.nrToneTransfer - 1.0f) > 1e-3f ||
        ...
        // at their defaults leave the neural result untouched.
        flags |= 64;
...
void Pipeline::Render(GpuContext& gpu, const SourceFrame& src, const Settings& s, ID3D12GraphicsCommandList* cmd, bool fresh, bool sourceChanged) {
    ...
    // The neural pass runs at the input size, and upscales to the output size when neural upscaling is on.
    const UINT nrInW = m_inW;
    const UINT nrInH = m_inH;
    const UINT nrOutW = m_outW;
    const UINT nrOutH = m_outH;
    ...
    if (fresh || !m_hasDisplay) {
        ...
        if (guidanceIdle) {
            ...
            if (nrWanted && EnsureNeuralTextures(gpu, nrInW, nrInH, nrOutW, nrOutH)) {
        ...
        if (dlaaOk) processed = &m_dlaaOut;
        if (nrWanted && !m_nrFailed && NeuralCreated() && EnsureNeuralTextures(gpu, nrInW, nrInH, nrOutW, nrOutH)) {
            neuralBase = processed;
            if (m_nrDirty) {
                neuralIn = &PrepareNeuralInput(gpu, cmd, s, *processed);
