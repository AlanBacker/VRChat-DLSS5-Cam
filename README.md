...
- **Neural pass active, but the picture is black or unchanged** – the app compares every neural frame with its input; the Neural section shows *Output change* and warns when the runtime reports success but delivers a black or unchanged picture (`log.txt`: "DLSSNR output check"). Restart the app; if it persists, that runtime build does not produce a picture on this GPU. Switching DLSS 5 off and on no longer re-creates the feature.
